...
const { sendCmd, queueState, historyState, favorites, currentTrackPath, toggleFavorite, switchTab, librarySearchQuery, haptic, showToast, pauseAfterPath } = usePlayer()
...
<template>
	<div v-if="ctxMenu.visible" class="fixed inset-0 z-[9998]" @click.self="closeCtxMenu"
		...
		<div class="ctx-menu absolute" :style="{ top: ctxMenu.y + 'px', left: ctxMenu.x + 'px' }">
			<div class="ctx-menu-header truncate">
			...
			<template v-if="ctxMenu.source === 'library'">
				<div class="ctx-menu-item" @click="ctxPlayNext">
					<i class="material-icons">queue_play_next</i> Que suene de próximo
				</div>
				<div class="ctx-menu-item" @click="ctxPlayNow">
					<i class="material-icons">play_arrow</i> Mandale play de una
				</div>
				<div class="ctx-menu-item" @click="ctxAddToQueue">
					<i class="material-icons">playlist_add</i> Al fondo de la fila
				</div>
				<div class="ctx-menu-item" @click="ctxToggleFavorite">
					<i class="material-icons">{{ favorites?.includes(ctxMenu.track?.path) ? 'favorite_border' : 'favorite' }}</i>
					{{ favorites?.includes(ctxMenu.track?.path) ? 'Sacar de favoritos' : 'A los favoritos' }}
				</div>
				<div class="ctx-menu-item" @click="ctxFilterByArtist">
					<i class="material-icons">person_search</i> Chusmear más del artista
				</div>
				<div class="ctx-menu-item" @click="ctxPauseAfter">
					<i class="material-icons">timer</i> Frenar la chata tras este tema
				</div>
			</template>
			...
			<template v-if="ctxMenu.source === 'queue'">
				<div class="ctx-menu-item" @click="ctxPlayNextQueue">
					<i class="material-icons">vertical_align_top</i> Subir a próximo
				</div>
				<div class="ctx-menu-item" @click="ctxPlayNow">
					<i class="material-icons">play_arrow</i> Mandale play de una
				</div>
				...
				<div class="ctx-menu-item delete-item" @click="ctxRemoveFromQueue">
					<i class="material-icons text-red-400">delete</i> <span class="text-red-400">Pegarle un voleo</span>
				</div>
			</template>
			...
			<template v-if="ctxMenu.source === 'history'">
				<div class="ctx-menu-item" @click="ctxHistoryPlayAgain">
					<i class="material-icons">add_circle_outline</i> Agregar a la fila
				</div>
				<div class="ctx-menu-item" @click="ctxToggleFavorite">
					<i class="material-icons">{{ favorites?.includes(ctxMenu.track?.path) ? 'favorite_border' : 'favorite' }}</i>
					{{ favorites?.includes(ctxMenu.track?.path) ? 'Sacar de favoritos' : 'A los favoritos' }}
				...
				<div class="ctx-menu-item delete-item" @click="ctxRemoveFromHistory">
					<i class="material-icons text-red-400">delete</i> <span class="text-red-400">Borrar del historial</span>
				</div>
			</template>
